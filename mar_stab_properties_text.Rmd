...
Often we would like to examine the potential effects of external drivers (covariates) on community dynamics. Traditionally, a MARSS model with covariate effects is defined as
...
However, when the MARSS model is defined this way, the estimated covariate effects in $\hat{\mathbf{C}}$ will be biased in the sense that they will not related directly to the relationship between $\mathbf{x}$ and $\mathbf{c}$. That is, if we plotted covariate $c_j$ against $x_i$, the estimated slope of the relationship would not equal the $ij$-th element of $\hat{\mathbf{C}}$. For example, if we knew that temperature had an effect 
