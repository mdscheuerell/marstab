...
There is growing interest in the use of first-order vector autoregressive models in ecology where they are often referred to as multivariate autoregressive, or MAR(1), models\footnote{Ives AR $et \ al$. (2003) $Ecol~Monogr$ 73:301–330}. In particular, MAR(1) models have been used to estimate interaction strengths among various members in a food web and evaluate overall community stability. The general form of a MAR(1) model is
...
Among the many ways of classifying stability, I am interested in the extent to which community interactions, relative to environmental forcing, contribute to the overall variance of the stationary distribution. Here it's helpful to first consider a one-dimensional system given by

\begin{equation} \label{eq9}
x_t = b x_{t-1} + w_t
\end{equation}

where $b$ is the strength of density dependence, the stationary variance of $x_t$ is $\sigma$, and the variance of $w_t$ is $q$. Analogous to Eqn (5), the variance of both sides of Eqn (\ref{eq9}) is given by

\begin{equation} \label{eq10a}
\sigma = b^2 \sigma + q
\end{equation}

such that the proportion of $\sigma$ owing to the one species' interaction with itself is then

\begin{equation} \label{eq10}
\frac{\sigma - q}{\sigma} = b^2.
\end{equation}


From Eqn (6) we can see that the species interactions strengths in $\mathbf{B}$ magnify the variable effects of the environment in $\mathbf{Q}$. Thus, the difference in the "volumes" of $\mathbf{\Sigma}$ and $\mathbf{Q}$ gives an indication of the degree to which species interactions inflate the environmental variance.

Here we will use determinants to measure the volume of a matrix. Looking back to (\ref{eq5}), we see that
...
The first follows Ives $et~al$.\footnote{Ives AR $et \ al$. (2003) $Ecol~Monogr$ 73:301–330}, where reactivity is based upon estimates of the process covariance $\mathbf{Q}$ and stationary covariance $\mathbf{\Sigma}$. Specifically, they define reactivity $\nu$ as
