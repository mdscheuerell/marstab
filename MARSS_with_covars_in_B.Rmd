...
If all of the covariates have been scaled to have zero mean and unit variance, then the observation equation is rewritten as
...
If the covariates have not been scaled, then the state-space model must instead be written with an additional vector $\mathbf{a}^*$ in the observation equation, such that

$$
\mathbf{y}_t^* = \mathbf{x}_t^* + \mathbf{a}^* + \mathbf{v}_t^* ~ \text{with} ~ \mathbf{v}_t^* \sim \text{MVN}(\mathbf{0}, \mathbf{R}^*),
$$

and all of the other terms are the same as above. To allow for non-zero means in the covariates, a $p \times 1$ vector $\mathbf{a}$ containing covariate means must be included in the $(n + p) \times 1$ column vector $\mathbf{a}^*$ , such that
...
As with $\mathbf{S}$ below, the covariate-specific means are specified rather than estimated.
